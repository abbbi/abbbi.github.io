...
A Proxmox backup server datastore includes the same pcat1 file as blob
index (.pcat.didx). So to actually beeing able to tell which backup contains
which files, one needs to:

1) Open the .pcat1.didx file and find out required blobs

2) Reconstruct the .pcat1 file from the blobs

3) Parse the pcat1 file and voila, you've got a directory listing.
